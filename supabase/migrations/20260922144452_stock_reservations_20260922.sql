...
declare
  ...
  v_active_stock_reservations integer;
  v_expired_active_reservations integer;
  v_pending_mp_without_reservation integer;
  ...
  v_reservation_function boolean;
  v_available_stock_function boolean;
begin
  if not public.request_is_service_role() then
    raise exception 'SERVICE_ROLE_REQUIRED';
  end if;

  select count(*)::integer into v_orders_without_items
  from public.orders o
  where not exists (select 1 from public.order_items oi where oi.order_id = o.id);

  select count(*)::integer into v_active_orders_without_items
  from public.orders o
  where o.status <> 'CANCELLED'
    and not exists (select 1 from public.order_items oi where oi.order_id = o.id);

  select count(*)::integer into v_legacy_cancelled_orders_without_items
  from public.orders o
  where o.status = 'CANCELLED'
    and not exists (select 1 from public.order_items oi where oi.order_id = o.id);

  select count(*)::integer into v_duplicate_keys
  from (
  ...
  select count(*)::integer into v_orders_multiple_payments
  from (
    select order_id
    from public.payments
    group by order_id
  ...
  select count(*)::integer into v_provider_payment_duplicates
  from (
  ...
  select count(*)::integer into v_stale_pending_orders
  from public.orders
  where status = 'PENDING_PAYMENT'
    and created_at < now() - interval '2 hours';

  select count(*)::integer into v_stuck_payment_events
  from public.payment_events
  where status = 'RECEIVED'
    and created_at < now() - interval '10 minutes';

  select count(*)::integer into v_paid_order_mismatches
  from public.payments p
  join public.orders o on o.id = p.order_id
  where p.status = 'PAID'
    and o.status not in (
  ...
  select count(*)::integer into v_active_stock_reservations
  from public.stock_reservations
  where status = 'ACTIVE'
    and expires_at > now();

  select count(*)::integer into v_expired_active_reservations
  from public.stock_reservations
  where status = 'ACTIVE'
    and expires_at <= now();

  select count(*)::integer into v_pending_mp_without_reservation
  from public.orders o
  ...
  v_finalize_payment_function := to_regprocedure('public.finalize_paid_order(uuid,text,jsonb)') is not null;
  v_reservation_function := to_regprocedure('public.reserve_order_stock(uuid,integer)') is not null;
  v_available_stock_function := to_regprocedure('public.get_product_available_stock(uuid[])') is not null;
  ...
  return jsonb_build_object(
    ...
    'active_stock_reservations', v_active_stock_reservations,
    'expired_active_reservations', v_expired_active_reservations,
    'pending_mp_without_reservation', v_pending_mp_without_reservation,
    'atomic_checkout_function', v_atomic_function,
    'finalize_payment_function', v_finalize_payment_function,
    'stock_reservation_function', v_reservation_function,
    'available_stock_function', v_available_stock_function,
