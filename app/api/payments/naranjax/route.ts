...
export async function GET(request: Request) {
  const limit = await distributedRateLimit(getRequestKey(request, "naranjax-status"), 60, 60_000);
...
export async function POST(request: Request) {
  const limit = await distributedRateLimit(getRequestKey(request, "naranjax-create"), 8, 60_000);
