...
function inferredMaterialPhoto(product: Pick<Product, "slug" | "sku" | "image_url"> & Partial<Pick<Product, "name" | "brand" | "category" | "subcategory">>) {
  ...
  if (/(tornillo|tarugo|fijacion|fijacion|mecha|punta|autoperforante)/.test(text)) return materialPhotos.screwsFasteners;
  if (/(masilla|cinta|enduido|junta|tomada|sellador)/.test(text)) return materialPhotos.jointCompound;
  if (/(lana|aislante|aislacion|membrana|acustica|termica|telgopor|poliestireno)/.test(text)) return materialPhotos.insulation;
  if (/(cementicia|aquaboard|superboard|exterior|extra resist|resist)/.test(text)) return materialPhotos.cementBoard;
  if (/(osb|fenolico|madera|placa estructural)/.test(text)) return materialPhotos.osbWood;
  if (/(cemento|arena|cal|plasticor|mortero|adhesivo)/.test(text)) return materialPhotos.cement;
  if (/(cable|termica|disyuntor|electric|ficha|caja)/.test(text)) return materialPhotos.electric;
  if (/(cano|caño|ppr|plomeria|agua|desague|tubo)/.test(text)) return materialPhotos.plumbing;
